...
int init() {
	...
	selector = new WorldSelector("UnigineVFX/vfx/");
	...
	track = tracker.loadTrack("UnigineVFX/vfx/tracks/vfx_jet_missiles.track");
	...
	reset_animation.getEventClicked().connect(functionid(reset_clicked));
	...
	toggle_animation.getEventClicked().connect(functionid(toggle_clicked));
